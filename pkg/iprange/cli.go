...
func runCLIV4(ctx context.Context, stdout, stderr io.Writer, stdin io.Reader, args []string) int {
	runner := newCLIV4Runner(stdout, stderr, stdin)
...
func (r *cliV4Runner) parseArgs(ctx context.Context, args []string) (int, bool) {
	...
	if len(r.before) == 0 {
		set, err := ParseReader(ctx, DefaultName, r.stdin, r.parseOpts)
		if err != nil {
...
func (r *cliV4Runner) parseArg(ctx context.Context, args []string, i int) (int, int, bool) {
	return parseCLIArg(ctx, args, i, r)
}

func (r *cliV4Runner) parseAlias(args []string, i int) (int, int, bool) {
	if i+1 >= len(args) {
...
func (r *cliV4Runner) parseMinPrefix(args []string, i int) (int, int, bool) {
...
func (r *cliV4Runner) parsePrefixes(args []string, i int) (int, int, bool) {
	...
	r.printOpts.PrefixesEnabled[32] = true
	return next, 0, false
}

func (r *cliV4Runner) parseDefaultPrefix(args []string, i int) (int, int, bool) {
...
func (r *cliV4Runner) parseReduceFactor(args []string, i int) (int, int, bool) {
...
func (r *cliV4Runner) parseReduceEntries(args []string, i int) (int, int, bool) {
...
func (r *cliV4Runner) parsePrintPrefix(args []string, i int) (int, int, bool) {
...
func (r *cliV4Runner) parsePrintPrefixIPs(args []string, i int) (int, int, bool) {
...
func (r *cliV4Runner) parsePrintPrefixNets(args []string, i int) (int, int, bool) {
...
func (r *cliV4Runner) parsePrintSuffix(args []string, i int) (int, int, bool) {
...
func (r *cliV4Runner) parsePrintSuffixIPs(args []string, i int) (int, int, bool) {
...
func (r *cliV4Runner) parsePrintSuffixNets(args []string, i int) (int, int, bool) {
...
func (r *cliV4Runner) parseDNSThreads(args []string, i int) (int, int, bool) {
...
func (r *cliV4Runner) loadInput(ctx context.Context, arg string) (int, bool) {
	sets, err := loadInputArgument(ctx, arg, r.stdin, r.parseOpts)
	if err != nil {
		return cliError(r.stderr, err), true
	}
	if r.readSecond {
...
func (r *cliV4Runner) execute() int {
	switch r.mode {
	case modeCombine, modeReduce:
		merged, err := mergeAll(r.before)
		if err != nil {
			return cliError(r.stderr, err)
		}
		if r.mode == modeReduce {
			r.printOpts.PrefixesEnabled = Reduce(merged, r.reduceFactor, r.reduceEntries, r.printOpts.PrefixesEnabled)
		}
...
func (r *cliV4Runner) writeSet(set *IPSet) int {
	if err := set.Write(r.stdout, r.printOpts); err != nil {
		return cliError(r.stderr, err)
	}
	return 0
}

func (r *cliV4Runner) executeCommon() int {
	if len(r.before) < 2 {
...
func (r *cliV4Runner) executeExcludeNext() int {
	if len(r.after) == 0 {
		return cliErrorText(r.stderr, "no files given after --exclude-next")
	}
	current, err := mergeAll(r.before)
	if err != nil {
		return cliError(r.stderr, err)
	}
...
func (r *cliV4Runner) executeDiff() int {
	if len(r.after) == 0 {
		return cliErrorText(r.stderr, "no files given after --diff")
	}
	left, err := mergeAll(r.before)
	if err != nil {
		return cliError(r.stderr, err)
	}
	right, err := mergeAll(r.after)
	if err != nil {
		return cliError(r.stderr, err)
	}
...
func (r *cliV4Runner) executeCompare() int {
	rows, err := CompareAll(r.before)
	if err != nil {
		return cliError(r.stderr, err)
	}
	r.writeCompareHeader()
	for _, row := range rows {
		_, _ = fmt.Fprintf(r.stdout, "%s,%s,%d,%d,%d,%d,%d,%d\n", row.Name1, row.Name2, row.Entries1, row.Entries2, row.Unique1, row.Unique2, row.CombinedIPs, row.CommonIPs)
	}
	return 0
}

func (r *cliV4Runner) executeCompareNext() int {
	rows, err := CompareNext(r.before, r.after)
	if err != nil {
		return cliError(r.stderr, err)
	}
	r.writeCompareHeader()
...
func (r *cliV4Runner) writeCompareHeader() {
	if r.header {
		_, _ = fmt.Fprintln(r.stdout, "name1,name2,entries1,entries2,ips1,ips2,combined_ips,common_ips")
	}
}

func (r *cliV4Runner) executeCompareFirst() int {
	rows, err := CompareFirst(r.before)
	if err != nil {
		return cliError(r.stderr, err)
	}
	if r.header {
	...
	for _, row := range rows {
		_, _ = fmt.Fprintf(r.stdout, "%s,%d,%d,%d\n", row.Name, row.Entries, row.UniqueIPs, row.CommonIPs)
	}
	return 0
}

func (r *cliV4Runner) executeCountUniqueMerged() int {
	row, err := CountUniqueMerged(r.before)
	if err != nil {
		return cliError(r.stderr, err)
	}
	if r.header {
	...
	_, _ = fmt.Fprintf(r.stdout, "%d,%d\n", row.Entries, row.UniqueIPs)
	return 0
}

func (r *cliV4Runner) executeCountUniqueAll() int {
	rows, err := CountUniqueAll(r.before)
	if err != nil {
		return cliError(r.stderr, err)
	}
	if r.header {

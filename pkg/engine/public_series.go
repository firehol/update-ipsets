...
func (e *Engine) writePublicHistoryCSV(name, outDir string) error {
	...
	return writeFileAtomicAt(filepath.Join(outDir, name+"_history.csv"), buf.Bytes(), 0o644, e.feedProcessingTimestamp(name))
}
...
func (e *Engine) writePublicChangesetsCSV(name, outDir string) error {
	...
	return writeFileAtomicAt(filepath.Join(outDir, name+"_changesets.csv"), buf.Bytes(), 0o644, e.feedProcessingTimestamp(name))
}
...
func (e *Engine) writePublicRetentionJSON(name, outDir string) error {
	...
	return writeFileAtomicAt(filepath.Join(outDir, name+"_retention.json"), data, 0o644, e.feedProcessingTimestamp(name))
}
...
func normalizeChangesetLedgerHeader(path string) error {
	...
	return writeFileAtomic(path, next, 0o644)
}

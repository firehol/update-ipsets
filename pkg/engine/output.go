...
func (e *Engine) sanitizeComparisonArtifacts(outDir string) error {
	...
	for rel, srcPath := range paths {
		...
		if err := writeFileAtomic(filepath.Join(outDir, rel), data, 0o644); err != nil {
			return err
		}
		cleaned++
	}
...
func (e *Engine) writePublicMetadataFiles(outDir string, outputNames []string) ([]string, error) {
	...
	if err := writeFileAtomic(filepath.Join(outDir, "robots.txt"), []byte(renderRobotsTXT(siteBase)), 0o644); err != nil {
		return nil, err
	}
	if err := writeFileAtomic(filepath.Join(outDir, "llms.txt"), []byte(renderLLMSTXT(siteBase, feedPrefix, outputNames)), 0o644); err != nil {
		return nil, err
	}
...
func (e *Engine) writeSitemapFiles(outDir, siteBase, feedPrefix string, outputNames []string) ([]string, error) {
	...
	if siteBase == "" {
		...
		if err := writeFileAtomic(filepath.Join(outDir, indexName), payload, 0o644); err != nil {
			return nil, err
		}

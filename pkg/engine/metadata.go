...
func (e *Engine) renderHeader(name string, src *config.Source, hash string, finalSet *iprange.IPSet, sourceMTime time.Time) []byte {
	return e.renderHeaderWithRuntime(e.operationSnapshot().runtime, name, src, hash, finalSet, sourceMTime)
}

func (e *Engine) renderHeaderWithRuntime(rt Runtime, name string, src *config.Source, hash string, finalSet *iprange.IPSet, sourceMTime time.Time) []byte {
...
func (e *Engine) renderSetInfo(name string, entry *cache.Entry) string {
	...
	return renderSetInfoForConfigRuntime(snap.cfg, snap.runtime, name, entry)
}

func renderSetInfoForConfigRuntime(cfg *config.Config, rt Runtime, name string, entry *cache.Entry) string {
	...
	redistributable := isRedistributableForConfig(cfg, name)
	...
	if redistributable && entry.PublicURL != "" {
		return fmt.Sprintf("[%s](%s%s)|%s|%s hash:%s|%s|updated every %s from [this link](%s)\n",
			name, rt.WebURL, name, entry.Info, entry.IPV, entry.Hash, quantity, freq, entry.PublicURL)
	}
	return fmt.Sprintf("[%s](%s%s)|%s|%s hash:%s|%s|updated every %s\n",
		name, rt.WebURL, name, entry.Info, entry.IPV, entry.Hash, quantity, freq)
}

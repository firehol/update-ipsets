...
func (e *Engine) writeCriticalInfrastructureForFeed(name string, datasets *criticalDatasets, outDir string, setCache *latestSetCache) ([]string, error) {
	...
	for _, providerName := range datasets.Names {
		...
		if checkErr := checkFileSetErr(provider.Set, providerName, e.logger); checkErr != nil {
			return nil, checkErr
		}

		var criticalIPs uint64
		hasOverlap := false
		for r := range iprange.IntersectIter(src.RangeSource, provider.Set) {
			criticalIPs += r.Size()
			if !hasOverlap {
				hasOverlap = true
				tierProviderCounts[provider.Meta.Tier]++
				if tierSets[provider.Meta.Tier] == nil {
					tierSets[provider.Meta.Tier] = iprange.New(name + "_critical_" + provider.Meta.Tier)
				}
			}
			if err := totalSet.AddRange(r); err != nil {
				return nil, fmt.Errorf("critical infrastructure aggregate add for %s/%s: %w", name, providerName, err)
			}
			if err := tierSets[provider.Meta.Tier].AddRange(r); err != nil {
				return nil, fmt.Errorf("critical infrastructure tier aggregate add for %s/%s: %w", name, providerName, err)
			}
		}
		if checkErr := checkFileSetErr(src.RangeSource, name, e.logger); checkErr != nil {
			return nil, checkErr
		...
		payload := criticalProviderOverlapJSON{
			...
			ProviderSetID: datasets.ProviderSetID,
			FeedIPs:       feedIPs,
			CriticalIPs:   criticalIPs,
		}
		if feedIPs > 0 {
			payload.Percent = 100 * float64(criticalIPs) / float64(feedIPs)
		}
		if err := e.writeCriticalProviderPayload(outDir, name, providerName, payload); err != nil {
			return nil, err
		}
		if criticalIPs == 0 {
			continue
		}
		providerPayloads = append(providerPayloads, payload)
	}

	sort.Slice(providerPayloads, func(i, j int) bool {
		if providerPayloads[i].CriticalIPs != providerPayloads[j].CriticalIPs {
			return providerPayloads[i].CriticalIPs > providerPayloads[j].CriticalIPs
		}
		return providerPayloads[i].Provider.Name < providerPayloads[j].Provider.Name
	})

	totalSet.Optimize()
	payload := criticalAggregateJSON{
		Feed:                name,
		Family:              criticalFeedFamily(e.cfg, name),
		FeedIPs:             feedIPs,
		CriticalIPs:         totalSet.UniqueCount(),
		Complete:            len(datasets.Missing) == 0,
		ProviderSetID:       datasets.ProviderSetID,
		ConfiguredProviders: append([]string(nil), datasets.Configured...),
		MissingProviders:    append([]criticalMissingProviderJSON(nil), datasets.Missing...),
		Providers:           providerPayloads,
	}
	if feedIPs > 0 {
		payload.Percent = 100 * float64(payload.CriticalIPs) / float64(feedIPs)
	}
	for _, tier := range config.CriticalTiers() {
		tierSet := tierSets[tier]
		if tierSet == nil {
			continue
		}
		tierSet.Optimize()
		...
		summary := criticalTierSummaryJSON{
			Tier:        tier,
			CriticalIPs: count,
			Providers:   tierProviderCounts[tier],
		}
		if feedIPs > 0 {
			summary.Percent = 100 * float64(count) / float64(feedIPs)
		}
		payload.Tiers = append(payload.Tiers, summary)
	}
	asnContext, err := e.criticalASNContextForFeed(name, feedIPs, outDir)
	if err != nil {
		return nil, err
	}

...
- Costa wants a terminal state for feeds that are no longer live.
- Archived feeds must never be retried automatically.
- Costa is now considering `archived` as a health state that is entered
  automatically after a feed remains `unavailable` for more than 2 months.
- Costa also wants operators to be able to trigger a manual health check, and a
...
- Costa is now reconsidering raw/public archived URLs and is leaning toward
...
- Costa clarified that "all URLs" means:
...
- Live verification on 2026-04-22 shows a likely remaining runtime gap:
  ...
  - Costa expects materially more feeds to have crossed the archival threshold
...
- UI follow-up:
  - the admin heartbeat/cards layout currently leaves `unmaintained` alone on a
    separate row
  - Costa wants the health cards to stay on the same row with the others
...
- Single-source retention derivatives already reuse the parent's effective
  ...
  - this means Costa's "single source derived health follows the source"
...
- Disabled is not strong enough for Costa's requirement:
...
- Current admin exposure is incomplete relative to Costa's new requirement:
...
- `deleted:` is stronger than needed here:
  ...
  - That does not fit Costa's requirement to preserve the current state and stop
    changing it.
...
- Costa currently recognizes only two non-health feed-control flags as meaningful:
  - `hidden` = do not show on the public UI
  - `disabled` = do not process this again
- Costa challenged the current use of `historical` and wants the state model
  clarified before implementation proceeds.
- New spec requirement from Costa:
...
- Costa is considering `archived` as part of feed health, not only as a
...
1. **What the source of truth for `archived` is**
   - Fact:
     - the earlier "config-curated `archived: true`" idea is now in conflict
       with Costa's chosen model where `archived` is an automatic health state
   ...
   - C. `archived` is manual/config only
     - this would contradict Costa's chosen automatic-health model
   - Decision: `A`
   - Why:
     - it matches Costa's current design exactly
     - it avoids conflicting state sources
     - Costa already reserved `disabled` for manual operator control
...
2. **What archived does to manual actions**
   ...
   - Decision: `D`
   - Why:
     - Costa confirmed that explicit operator intent is sufficient guardrail for
...
4. **Which public URLs archived feeds should disable**
   ...
   - Decision: `A`
   - Why:
     - Costa clarified that archived feeds are historical/reference entries, not
...
5. **How derived feeds behave when a parent is archived**
   ...
   - Decision: `A`
   - Why:
     - Costa has now made the precise derivative and merge rules explicit.
...
6. **Whether auto-archival exists**
   ...
   - Decision: `D`
   - Why:
     - This matches Costa's latest direction more closely.
...
7. **Can we reuse `historical` instead of adding `archived`**
   ...
   - Decision: `D`
   - Why:
     - Costa's current model already has:
...
8. **What integrity should do for archived public feeds**
   ...
   - Why:
     - This matches Costa's latest direction and UX rationale.
...
10. **How derived feeds behave when a parent becomes archived**
   ...
   - Decision: `B`
   - Why:
     - Costa explicitly selected this boundary.
...
11. **How the operator-triggered archived health probe should surface in the UI/API**
   ...
   - Decision: `D`
   - Why:
     - Costa confirmed that the existing actions are sufficient.
...
- Archived must be different from disabled:
  ...
  - archived, under Costa's current proposal, is an automatically-derived trust
...
- Costa's `1A` decision must apply to every public path that gives the user the

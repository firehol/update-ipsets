...
> editorial decisions. Costa's words:
>
...
> Costa anything. Fix what has an already-stated answer. Only present
...
- **18 are autonomous fixes** — prior decisions exist, or the fix is a
  ...
  more input.
- **6 need Costa's input** — new design territory or explicit
  open-ended direction ("home page from scratch").
- **1 is missing** — Costa skipped #13 in his numbering.

2026-04-23 extension — search-surface consistency:

- Costa requires the feed-scoped IP search surface to use the **same
  component** the homepage uses, not a separate variant that drifts in
  layout or data rendering.
- Costa also requires the homepage IP lookup result rendering to show
...
- Costa reported three concrete homepage regressions:
...
- Costa reported three additional regressions / polish gaps:
...
- Costa requires the overlap list to state the health of each peer feed.
- Costa also requires a warning when a feed that is itself not `archived` or
...
- Costa requires ASN visualizations to behave like the country maps:
...
- Costa reported that both public classification-detail pages are too
...
- If Costa wants an ASN country map, the backend must either:
...
Costa decisions recorded on 2026-04-24:
...
- Costa reported that the country and ASN detail pages can become
...
- Costa reported four regressions after the first bounded-viewport pass:
...
- Costa clarified that the six homepage lens tiles should not become merely
...
- Costa requires the homepage explorer's `By maintainer` view to be
  alphabetical.
...
- Costa requires the homepage lens tiles to unselect once the user manually
...
- Costa confirmed the right-column background evolution chart placement is
  now correct.
- New issue:
...
- Legacy age/retention charts explicitly disclosed **partial /
  ...
  - `/home/costa/src/firehol/firehol/html/ipsets/index.html:1179-1183`
  - `/home/costa/src/firehol/firehol/html/ipsets/index.html:1266-1273`
  - `/home/costa/src/firehol/firehol/html/ipsets/index.html:1284-1286`
...
- Legacy age chart aged the histogram forward to **now** using the
  ...
  - `/home/costa/src/firehol/firehol/html/ipsets/index.html:1168-1176`
  - `/home/costa/src/firehol/firehol/html/ipsets/index.html:294-298`
...
- Legacy rendered zero-hour buckets as **`< 1`**, preventing false
  precision:
  - `/home/costa/src/firehol/firehol/html/ipsets/index.html:1185`
  - `/home/costa/src/firehol/firehol/html/ipsets/index.html:1274`
...
- Legacy charts used specific explanatory boxes for **empty**, **cannot
  ...
  - legacy:
    - `/home/costa/src/firehol/firehol/html/ipsets/index.html:1159`
    - `/home/costa/src/firehol/firehol/html/ipsets/index.html:1263`
    - `/home/costa/src/firehol/firehol/html/ipsets/index.html:1357-1358`
    - `/home/costa/src/firehol/firehol/html/ipsets/index.html:1510-1524`
  - current:
...
(`#fafafa`) + a single solid red `#dc2626`. Costa's complaint #2
("background should be navy") is a **regression**, not a new ask.

**Quality reference**: HackTheBox. Costa quote: *"refined dark theme,
...
The old Alpine site also had `globe.gl` 3D globe on the hero — Costa
...
TODO-asn-providers.md) — Costa's complaint #9 is factually correct.
...
### Specs regression
Original Phase 3 design had an exhaustive 6-group spec sheet, but Costa
...
- keep Access / Processing out of the public spec table unless Costa
...
const churn = prev && prev.ips > 0
  ...
  : null;
```
Costa's complaint #11: net delta can be 0 while the list is completely
...
| 5  | Home page redesign from scratch | **D** | XL | Costa explicitly: *"put it as a separate TODO item"*. Prior storefront spec exists but is stale (globe.gl, Alpine). **Split to `TODO-HOMEPAGE.md`.** Needs direction: how much of old storefront to keep, globe or different hero, what stats live in the pulse strip. |
...
| 16 | Specs richer | **A** | M | Costa explicitly pruned the public table to 3 groups in commit `d049979`. The remaining work is to enrich those 3 groups with the missing public facts already in `FeedMetadata` (`provenance`, `attribution`, availability/status wording, etc.), not to restore Access/Processing. |
...
| 18 | Hero: evolution chart (background or right column) | **A**/**D** | M | Prior P1 decision: right-column cinematic chart. Costa now says *"it can be a background"*. **Autonomous to restore right-column version** (matches spec); needs input only if Costa prefers the background treatment. |
...
| 21 | Composition: treemap default with AS number + name | **A** | M | Current default is table. Spec had bubble pack default. Costa now overrides to treemap default. Treemap via `@visx/hierarchy` (already in deps). Labels: `AS{num} {name}`. |
...
| 26 | **NEW**: ASN provider tabs use ugly internal codes | **A** | S | Costa quote (April 7): *"the provider names of ASN selection are ugly internal codes, not friendly provider names, unlike the geo selection."* — `section-asn.tsx` and `section-geo.tsx` should render `provider.label` (already in YAML) instead of `provider.name`. |
| 27 | **NEW (verify)**: No placeholder/random values anywhere | **A** | S | Costa April 5: *"interesting. you have put random numbers everywhere. Nice best practice."* — audit React UI for hard-coded sample values that fall back to fake data. Replace with real "—" or "no data" treatment. |
| 28 | **NEW**: Cross-provider radar viz broken | **D** | M | Costa quote (April 7): *"Cross-provider radar... we have 4 configured and it does not work."* — Phase 3 spec listed `Cross-provider radar (when ≥2 ASN sources configured)` as Tab 3 of ASN section. Current React UI has no radar tab at all. Needs decision: (a) drop the radar entirely, (b) build it, (c) defer. |
| 29 | **NEW (backend bug)**: ASN data integrity | **D** | ? | Costa: *"I see an IP feed with 5k entries, and the ASN table reports hundreds of thousands of IPs."* — backend ASN attribution is over-counting somewhere. Not in the original list, but worth flagging. May have been fixed since; verify on a small feed. |

**Autonomous fixes**: 1, 2, 3, 4, 7, 8, 9, 10, 11, 12, 14, 15, 16, 19, 21, 22, 23, 24, 26, 27 → **20 items**

**Needs Costa input**: 5, 6, 17, 18, 20, 25, 28, 29 → **8 items**
...
Follow-up correction requested by Costa:
...
The "deterministic signals we discussed" that Costa says are missing
...
Action: spawn a follow-up explore agent on `~/.claude/projects/-home-costa-src-firehol-update-ipsets/86c989d2-33c8-4a1d-b9c7-8e0a58cbb28d.jsonl`
...
deterministic outputs Costa wanted shown. Specifically: anything
beyond the 16 insight rules already in `pkg/insights/`.

---

## Questions for Costa (the 8 items that need input)
...
4. **#18 Hero evolution** — Costa has expressed THREE different
   ...
   - 2026-04-23 clarification:
     - the current implementation now places the chart at the top of the
       right column, which is still wrong for Costa's intent here
     - Costa clarified he means **behind** the hero content, not
...
- 2026-04-23: Costa chose the principle-driven chart approach (`A`): derive truthfulness / interpretation-safety rules from the legacy site, codify them in specs, and apply them to all current public visualizations.
...
- 2026-04-23: Follow-up correctness issue reported by Costa after the
...
Implementation direction confirmed by Costa:
...
- 2026-04-23: New homepage correctness/layout issues reported by Costa:

...
Costa asked for a new `specs/` directory with:
...
- Ran external implementation-vs-spec reviews and manually classified the main candidates:
  ...
  - merge missing-input behavior: real spec gap, then fixed after Costa decided merges must fail
...
- It is useful as internal technical documentation, but it does not yet meet Costa's required bar for a true application contract / reimplementation spec.

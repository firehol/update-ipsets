...
## TL;DR
Costa reported that all `misp_*` feeds now show zero IPs. We need to find the regression, fix the current downloader/parser behavior, and remove the incorrect empty/zero history entries introduced by the bug so these feeds no longer appear to have gone empty.

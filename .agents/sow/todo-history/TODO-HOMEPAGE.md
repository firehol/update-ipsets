...
- user wants the homepage feed explorer to start with these health classes
  selected by default:
  - `healthy`
  - `delayed`
  - `risky`
  - `unavailable`
- user explicitly does **not** want these selected by default:
  - `archived`
  - `unmaintained`
  - `empty`
...
## Resolved items (user authorized the implementation agent to decide)

...
user clarified the intended runtime contract and these decisions are now
...
- user wants this panel to show only feeds that are actually in flight right
...
- user reports that this message never clears, even after the run view has
  otherwise moved on.
...
- user reports that a missing local output/source paired with a feed that is
...
- user questioned whether the repeatedly-valid design is not the ad-hoc
...
New design question raised by user on 2026-04-19:
...
- user explicitly accepts provider-specific schemes if that is the cleanest
...
3. Admin visibility of artifact parents
   - A. Separate admin inventory for artifact parents, not part of the feeds table.
     - Fits user's "do not mix concerns" requirement.
...
New framing raised by user on 2026-04-19:
...
2. Local-file ingestion syntax
   ...
   - user questioned whether this should be replaced by the standard-looking
     `file://...` URL scheme.
...
explicitly decided by user. The behavior was already decided earlier:
...
- A. Artifact refresh downloads the parent and materializes child staged source
  ...
  - Pros:
    ...
    - matches user's "external cronjob writes files, update-ipsets ingests
      them" framing
...
- A. Add a new top-level `artifacts:` section.
  - Pros:
    ...
    - matches user's "do not mix concerns" requirement
  - Cons:
...
Additional UX constraint raised by user on 2026-04-19:
...
- user's concern is configuration UX:
  - simple
  - predictable
  - clean
...
New design refinement raised by user on 2026-04-19:

- user wants one simple, predictable pattern for what a source URL may contain.
...
Current recommendation after user's UX clarification:
...
- user explicitly wants administrators to enable/disable the parent globally.
...
The current runtime still couples **download timing** to **processing timing** inside one `RunOnce()` execution. Even though there is a prefetch phase, the scheduler still waits for the whole fetch stage to complete before the processing stage starts. user wants these concerns separated completely:
...
### Downloader facts relevant to user's new requirement
...
### Already decided by user on 2026-04-19
...
2. **What happens if a feed changes multiple times before the next processing tick?**
   ...
   - **Decision: implied by user's dirty-feed rule → A**
...
3. **Should the changed-feeds queue survive daemon restart?**
   ...
   - **Decision: implied by user's `.new` startup-recovery rule → A**
...
**Option B (user's phrasing)**: retention-append is its own queue (Queue 3). But then dependents need to wait until Queue 3 finishes before Queue 2 can inject them. That breaks the "dynamic injection" requirement because the dependents can't be injected into Queue 2 — they'd have to go into a future Queue 2 which doesn't exist.

I recommend **Option A** and will use it unless user objects. The retention snapshot append is a single binary write per updated feed (~tens of milliseconds total across all feeds) — making it a separate queue is structural overhead without a memory benefit.

To user: does "the retention queue" you described mean a separate queue stage, or just "every updated feed appends its retention snapshot as part of its finalize step"? I suspect the latter.
...
## Decisions (confirmed by user on 2026-04-09)
...
- 2026-04-09: design approved by user, feasibility pass done, implementation complete through deploy.
...
## Phase 2 — blockers and integrity check (2026-04-09, user's follow-up)

user escalated these from "follow-ups" to "blockers" and added a new integrity-check requirement:

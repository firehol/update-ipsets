...
- **Do not install, do not restart any daemon.** Production deployment is user's call, not the agent's.
...
After the commit lands, user will install + restart + trigger a rebuild to validate in production. That's a separate step that only user authorizes.

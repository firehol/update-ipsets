...
user found a bug in the Go rewrite: the "IPs added vs removed per update" data must represent the last real content changes, not rebuilds or checks where the effective set did not change.
...
Evidence from `/home/user/src/firehol/firehol/sbin/update-ipsets`:
...
- [process.go](/home/user/src/firehol/update-ipsets/pkg/engine/process.go):310 skips equal processed sets only when `opts.Rebuild` is false.
- [process.go](/home/user/src/firehol/update-ipsets/pkg/engine/process.go):324 then calls `updateRetention()` even for rebuilds of identical content.
- [retention.go](/home/user/src/firehol/update-ipsets/pkg/engine/retention.go):61 appends `changesets.csv` unconditionally.
- [query.go](/home/user/src/firehol/update-ipsets/pkg/engine/query.go):117 returns every parsed row from `changesets.csv`.
- [server.go](/home/user/src/firehol/update-ipsets/pkg/web/server.go):253 documents the wrong behavior: "the total may be zero while the list was refreshed".
- Existing local generated data confirms pollution exists: `/home/user/.update-ipsets/lib/dshield/changesets.csv:3` and `/home/user/.update-ipsets/lib/bogons/changesets.csv:3` contain `0,0` rows.
...
- Bash `history_keep()` writes snapshots named from the processed file mtime and touches them to that mtime (`/home/user/src/firehol/firehol/sbin/update-ipsets:973-987`).
- Bash `history_cleanup()` deletes snapshots older than the current time minus the configured history window by comparing file mtimes to a reference file (`/home/user/src/firehol/firehol/sbin/update-ipsets:990-1002`).
...
- Bash public publication stages generated files in `RUN_DIR` and then uses `mv -f RUN_DIR/* WEB_DIR/` (`/home/user/src/firehol/firehol/sbin/update-ipsets:2473-2474`). This is not a single filesystem transaction; the Go rewrite now matches the staging-before-move pattern, not a stronger directory-swap design.

## External Reviewer Parity Audit — 2026-04-10

user requested independent checks from `glm`, `qwen`, `kimi`, and `minimax` for bash-vs-Go file and logic-per-file parity.
...
1. `StartedDate` is not bash-compatible.
   - Bash sets a missing `IPSET_STARTED_DATE` from `IPSET_SOURCE_DATE` (`/home/user/src/firehol/firehol/sbin/update-ipsets:2781`).
...
2. `clock_skew` JSON units are not bash-compatible.
   - Bash emits `clock_skew` multiplied by 1000 in per-feed JSON and `all-ipsets.json` (`/home/user/src/firehol/firehol/sbin/update-ipsets:1581`, `:1632`).
...
3. Go rewrites public metadata/series files for all output feeds on every run; bash writes web files only when there are updated sets or a forced rebuild.
   - Bash exits web update when no `UPDATED_SETS` and no `FORCE_WEB_REBUILD` (`/home/user/src/firehol/firehol/sbin/update-ipsets:2096-2099`), and changesets/retention loop only over `UPDATED_SETS` (`:2453-2464`).
...
4. Internal binary state filenames still differ.
   - Bash primary retention latest file is `lib/<name>/latest` (`/home/user/src/firehol/firehol/sbin/update-ipsets:1808`).
   - Go writes `lib/<name>/latest.set` (`pkg/engine/finalize.go:43`) while reading both names.
   - Bash retention new-IP files are `lib/<name>/new/<epoch>` (`/home/user/src/firehol/firehol/sbin/update-ipsets:1780-1781`).
...
5. Bash keeps `lib/<name>/histogram`; Go does not.
   - Bash writes a shell cache at `/home/user/src/firehol/firehol/sbin/update-ipsets:1940`.
...
7. Rename/delete cleanup parity is incomplete.
   - Bash rename/delete handles selected web suffixes including `_comparison.json`, hardcoded geo country files, `_history.csv`, `.json`, `.html` (`/home/user/src/firehol/firehol/sbin/update-ipsets:3208-3215`, `:3312-3318`).
...
- Non-redistributable `.setinfo` "disabled" mismatch was false: bash only omits the source link, matching Go (`/home/user/src/firehol/firehol/sbin/update-ipsets:2865-2871`, `pkg/engine/metadata.go:176-184`).

Recommended follow-up:
- Fix the seven surviving discrepancies above if user wants strict file parity.
...
user requested detailed verification of the reviewer claims and fixes for confirmed discrepancies.

Verified additional facts before implementation:

1. Literal per-feed JSON content parity is broader than the reviewer list.
   - Bash per-feed JSON includes legacy fields `grade`, `protection`, `intended_use`, `false_positives`, `poisoning`, and `services` (`/home/user/src/firehol/firehol/sbin/update-ipsets:1597-1603`).
   ...
   - This conflicts with the existing project rule in `AGENTS.md` that forbids phantom fields unless user explicitly decides to reintroduce them.

2. Literal comparison JSON content parity is broader than the reviewer list.
   - Bash `_comparison.json` rows have only `name`, `category`, `ips`, and `common` (`/home/user/src/firehol/firehol/sbin/update-ipsets:2267-2288`).
...
4. The reviewer list missed two additional bash contract gaps.
   - Bash supports `WEB_OWNER` chown for public web files and copied ipset files (`/home/user/src/firehol/firehol/sbin/update-ipsets:2473-2475`, `:895-920`). Go parses `web_owner` in config but drops it when resolving runtime (`pkg/config/config.go:104-108`, `pkg/engine/runtime.go:14-50`).
   - Bash copies updated redistributable ipset/netset files into `WEB_DIR_FOR_IPSETS` via `.new` then rename (`/home/user/src/firehol/firehol/sbin/update-ipsets:895-920`). Go only exposes `local_copy_url` in metadata; it does not copy the files.
...
- OBSTACLE FOUND for literal "no extra fields/files" parity:
  ...
  - Reintroducing bash phantom fields conflicts with the existing codebase rule unless user explicitly chooses that.

user decisions made on 2026-04-10 after the follow-up verification:
...
Lint cleanup requested by user:
...
user asked to fix the remaining verified bash-vs-Go file contract issues after the external reviewer run.
...
Do not remove Go-only web/API extension files or reintroduce bash phantom metadata fields; user already chose "legacy contract parity plus documented Go extensions" and no phantom fields.
...
Evidence:
- Bash creates `.gitignore` only in git repos and initializes it with `*.setinfo` and `*.source` (`/home/user/src/firehol/firehol/sbin/update-ipsets:728`).
- Bash appends ignored private files instead of replacing `.gitignore` (`/home/user/src/firehol/firehol/sbin/update-ipsets:797-804`).
- Bash `README.md` preserves `README-EDIT.md`, includes legacy generated prose and table headers, then appends `*.setinfo` (`/home/user/src/firehol/firehol/sbin/update-ipsets:831-842`).
- Bash timestamp script uses `#!/bin/bash`, includes a safety argument guard, and writes `touch --date=@...` commands (`/home/user/src/firehol/firehol/sbin/update-ipsets:858-867`).
- Bash writes `.setinfo` inside the git-only block (`/home/user/src/firehol/firehol/sbin/update-ipsets:2849-2873`).
...
user raised a broader concern after the zero-delta changeset fix: other maintained files may have similar Go-vs-bash contract drift, especially files with append/truncate behavior.
...
user decisions made on 2026-04-10:

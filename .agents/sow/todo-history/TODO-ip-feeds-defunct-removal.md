...
user asked to verify all feeds the licenses-research flagged as defunct, then
...
4. **Verification**:
   ...
   - Show the exact diff to user before declaring done.

## Hard rules (per CLAUDE.md)

- Never `git add -A`. Add only the modified files.
- No commit until user approves the diff.
